...
export function AboutUs() {
  return (
    <div
      id="aboutUs"
      className="relative min-h-screen bg-white text-gray-900 pt-20 overflow-hidden"
    >
      {/* Background accents */}
      <div className="absolute top-1/4 left-1/4 w-72 h-72 bg-[#00F000]/10 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 right-1/4 w-72 h-72 bg-[#00F000]/10 rounded-full blur-3xl" />
        ...
        <section className="mb-20">
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            {principlesData.map((principle, index) => (
              <motion.div
                key={index}
                className="bg-gray-50 p-8 rounded-[2rem] border border-gray-200 shadow-md hover:shadow-xl transition-shadow duration-500 flex flex-col h-full"
                initial={{ opacity: 0, y: 50 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.2 + 0.5 }}
                whileHover={{ scale: 1.02 }}
